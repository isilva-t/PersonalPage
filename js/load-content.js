...
function createProjectCard(project, isFeatured = false) {
    const hasLiveDemo = project.liveDemo ? true : false;
    
    return `
        <div class="project-card ${isFeatured ? 'featured' : ''} bg-light-navy rounded-lg overflow-hidden hover:transform hover:-translate-y-2 transition-all duration-300 flex-shrink-0">
            <div class="relative">
                <img src="${project.image}" alt="${project.title}" class="w-full h-48 object-cover" onerror="this.src='images/placeholder.png'">
                <div class="absolute top-4 right-4">
                    <span class="bg-navy/80 text-cyan text-xs font-mono px-3 py-1 rounded">${project.period}</span>
                </div>
            </div>
            <div class="p-6">
                <h3 class="text-xl font-bold text-lightest-slate mb-3">${project.title}</h3>
                <p class="text-slate text-sm mb-4 leading-relaxed">${project.shortDesc}</p>
                ...
                <div class="flex gap-3 text-sm">
                    <a href="${project.github}" 
                       class="text-cyan hover:text-lightest-slate transition-colors flex items-center gap-1" 
                       target="_blank" 
...
function createCarousel(projects, containerId, isFeatured = false) {
